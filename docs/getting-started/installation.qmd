...
```{.bash filename="Terminal"}
gh release download "{{< env VERSION >}}" --repo mcanouil/quarto-wizard --pattern "*.vsix"
gh attestation verify "quarto-wizard-{{< env VERSION >}}.vsix" --repo mcanouil/quarto-wizard
code --install-extension mcanouil.quarto-wizard
```
...
- VS Code's built-in GitHub authentication (offered automatically when an authentication error occurs).
