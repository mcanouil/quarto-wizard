...
This file declares every configurable option, shortcode parameter, format key, element attribute, and CSS class the extension accepts.
...
::: {.callout-warning}
Schema specification support in Quarto Wizard may evolve over time.
Quarto CLI may use, modify, or discard this schema behaviour in future releases.
No guarantees are provided regarding Quarto CLI native support.
:::
...
- `https://m.canouil.dev/quarto-wizard/assets/schema/v1/extension-schema.json` (v1, default; kebab-case canonical with camelCase aliases).
- `https://m.canouil.dev/quarto-wizard/assets/schema/v2/extension-schema.json` (v2; JSON Schema canonical names exclusively).

When the `$schema` key is omitted, the file is validated against v1.
...
| Array uniqueness         | Not available                             | `uniqueItems`                                                           |
...
rewrite kebab-case keyword names to their camelCase equivalents and replace `min` / `max` with `minimum` / `maximum`.

## Top-level Structure

The schema has six optional top-level sections.
...
An array of project type strings contributed by the extension.
These values are suggested for `project.type` in `_quarto.yml`.
...
CSS classes contributed by the extension's filters.
Each entry maps a class name to a descriptor containing a `description`.
Quarto Wizard uses this information to provide class name completion and hover documentation in fenced divs (`:::`), bracketed spans (`[]`), and code block attributes.
...
| `uniqueItems`           | `boolean`                     | Reject an array that repeats an item, for `type: array`. Items are compared by type and rendered value, so the number `1` and the string `"1"` are different items. |
...
- [`v1/extension-schema.json`](/assets/schema/v1/extension-schema.json) (JSON Schema draft 2020-12; dual-keyed vocabulary).
- [`v2/extension-schema.json`](/assets/schema/v2/extension-schema.json) (JSON Schema draft 2020-12; canonical camelCase vocabulary).

These are the same schemas referenced by the `$schema` key at the top of schema definition files.
They can be used by any JSON Schema-aware tool for validation and autocompletion.

The reference implementation of the v2 vocabulary is a Lua module, published at:

- [`v2/schema.lua`](/assets/schema/v2/schema.lua) (reference validator; reads `_schema.yml` and validates options at render time).

Quarto extensions load this module to validate their configuration during a render.
The module is standalone: it requires no other module, and it reaches `pandoc` and `quarto` only through its `_env` table.
Its `SCHEMA_VERSION` field holds the URL of the meta-schema it implements.

The vocabulary and the validator version together: version 2 of the vocabulary is implemented by version 2 of the module, and a future version 3 ships with its matching module.
That version is the version of the vocabulary, and not the version of the `@quarto-wizard/schema` package that publishes both.

Example instances showing every supported property are also available:

- [`v1/extension-schema-example.yml`](/assets/schema/v1/extension-schema-example.yml) (v1 YAML, kebab-case keys).
- [`v1/extension-schema-example.json`](/assets/schema/v1/extension-schema-example.json) (v1 JSON, camelCase keys).
- [`v2/extension-schema-example.yml`](/assets/schema/v2/extension-schema-example.yml) (v2 YAML, canonical names).
- [`v2/extension-schema-example.json`](/assets/schema/v2/extension-schema-example.json) (v2 JSON, canonical names).
