...
| `uniqueItems`           | `boolean`                     | Reject an array that repeats an item, for `type: array`. Items are compared by type and rendered value, so the number `1` and the string `"1"` are different items. v2 only. |
