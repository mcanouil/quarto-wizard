...
- [`extension-schema.json`](/assets/schema/v1/extension-schema.json) (JSON Schema draft 2020-12).

This is the same schema referenced by the `$schema` key at the top of schema definition files.
It can be used by any JSON Schema-aware tool for validation and autocompletion.

Example instances showing every supported property are also available:

- [`extension-schema-example.yml`](/assets/schema/v1/extension-schema-example.yml) (YAML example, kebab-case keys).
- [`extension-schema-example.json`](/assets/schema/v1/extension-schema-example.json) (JSON example, camelCase keys).
