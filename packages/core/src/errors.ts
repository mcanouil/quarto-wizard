...
export class AuthenticationError extends QuartoWizardError {
  constructor(message: string) {
    super(
      message,
      "AUTH_ERROR",
      "Provide a GitHub token via GITHUB_TOKEN environment variable or auth option"
    );
...
export class RepositoryNotFoundError extends QuartoWizardError {
  constructor(message: string, hint?: string) {
    super(
      message,
      "NOT_FOUND",
      hint ?? "Check if the repository exists and you have access to it"
    );
...
export class ManifestError extends QuartoWizardError {
  ...
  constructor(message: string, manifestPath?: string) {
    super(
      message,
      "MANIFEST_ERROR",
      manifestPath ? `Check the manifest file at: ${manifestPath}` : undefined
    );
