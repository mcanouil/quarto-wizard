...
export function validateManifest(
  ...
  if (requireContributions) {
    ...
    if (!hasContributions) {
      issues.push({
        ...
        suggestion:
          "Add filters, shortcodes, formats, or other contributions to _extension.yml",
      });
    }
  }
  ...
  if (quartoVersion && manifest.quartoRequired) {
    const compatible = checkQuartoCompatibility(
 ...
 */
function checkQuartoCompatibility(
  required: string,
  current: string
): boolean {
...
export function formatValidationIssues(result: ValidationResult): string {
  ...
  const lines: string[] = [];
  lines.push(
    `Found ${result.summary.errors} error(s) and ${result.summary.warnings} warning(s):`
  );
  lines.push("");
