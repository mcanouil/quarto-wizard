...
describe("remove", () => {
  ...
  it("should clean up empty owner directory", async () => {
    ...
    await remove(
      { owner: "single-owner", name: "only-ext" },
      { projectDir: tempDir, cleanupEmpty: true }
    );
  ...
  it("should respect cleanupEmpty = false", async () => {
    ...
    await remove(
      { owner: "owner", name: "ext" },
      { projectDir: tempDir, cleanupEmpty: false }
    );
...
describe("removeMultiple", () => {
  ...
  it("should remove multiple extensions", async () => {
    ...
    expect(
      fs.existsSync(path.join(tempDir, "_extensions", "quarto-ext", "fontawesome"))
    ).toBe(false);
    expect(
      fs.existsSync(path.join(tempDir, "_extensions", "quarto-ext", "lightbox"))
    ).toBe(false);
    expect(
      fs.existsSync(path.join(tempDir, "_extensions", "other", "extension"))
    ).toBe(true);
  });
